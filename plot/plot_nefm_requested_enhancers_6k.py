...
ATAC_PATH = ROOT / "data" / "selected_rectangle" / "GW12_spatial_ATAC_rectangle_6863spots.h5ad"
LINK_PATH = ROOT / "data" / "GW12_region_to_gene_links_filtered.tsv"
OUT_PATH = ROOT / "plot" / "NEFM_requested_enhancers_6k.png"
...
RNA_CMAP = LinearSegmentedColormap.from_list(
    "rna_6k", ["#F7F7F7", "#DCEAF7", "#6BAED6", "#2171B5", "#08306B"], N=256
)
ATAC_CMAP = LinearSegmentedColormap.from_list(
    "atac_6k", ["#EDEDED", "#FFF7F3", "#FDE0DD", "#FCC5C0", "#FA9FB5", "#F768A1", "#DD3497", "#AE017E"], N=256
)
...
def draw_panel(ax, coords, values, title, cmap, vmax):
    ax.scatter(
        coords[:, 0], coords[:, 1], c=values, s=3.2, cmap=cmap,
        norm=Normalize(0, vmax), linewidths=0, rasterized=True
    )
...
def main():
    ...
    if not np.array_equal(rna.obs_names.to_numpy(), atac.obs_names.to_numpy()):
        raise ValueError("6k RNA and ATAC obs order differs")
    ...
    axes = axes.ravel()
    draw_panel(axes[0], coords, expr, "A  NEFM RNA", RNA_CMAP, expr_vmax)
    ...
    for i, enhancer in enumerate(REQUESTED_ENHANCERS):
        ...
        draw_panel(axes[i + 1], coords, enh[:, i], title, ATAC_CMAP, enh_vmax)
    ...
    fig.suptitle("6k spatial maps for NEFM and the requested enhancers", x=0.02, ha="left", fontsize=10.5, fontweight="bold")
