...
RNA_PATH = ROOT / "data" / "selected_8k" / "GW12_spatial_RNA_8065spots.h5ad"
...
OUT_PATH = ROOT / "plot" / "NEFM_requested_enhancers_8k.png"
...
RNA_CMAP = LinearSegmentedColormap.from_list(
    "rna_8k", ["#F7F7F7", "#DCEAF7", "#6BAED6", "#2171B5", "#08306B"], N=256
)
ATAC_CMAP = LinearSegmentedColormap.from_list(
    "atac_8k", ["#EDEDED", "#FFF7F3", "#FDE0DD", "#FCC5C0", "#FA9FB5", "#F768A1", "#DD3497", "#AE017E"], N=256
)
...
def draw_panel(ax, coords, values, title, cmap, vmax):
    ax.scatter(
        coords[:, 0], coords[:, 1], c=values, s=3.0, cmap=cmap,
...
def main():
    ...
    if not np.array_equal(rna.obs_names.to_numpy(), atac.obs_names.to_numpy()):
        raise ValueError("8k RNA and ATAC obs order differs")
    ...
    fig.suptitle("8k spatial maps for NEFM and the requested enhancers", x=0.02, ha="left", fontsize=10.5, fontweight="bold")
