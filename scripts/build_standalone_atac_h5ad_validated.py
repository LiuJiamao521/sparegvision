...
VALIDATED_TXT = LOG_DIR / 'csv_intervals.validated_22475.txt'
INVALID_TXT = LOG_DIR / 'csv_intervals.invalid_22475.txt'
...
def load_intervals(limit: int | None) -> list[str]:
    vals = sorted(
        p.stem for p in CSV_DIR.iterdir()
        if p.is_file() and p.suffix == '.csv' and len(p.stem.split('-')) == 3
    )
...
def validate_intervals(intervals: list[str], n_obs: int, progress_every: int) -> tuple[list[str], list[str]]:
    valid = []
    invalid = []
    total = len(intervals)
    for i, interval in enumerate(intervals, start=1):
        path = CSV_DIR / f'{interval}.csv'
        try:
            text = path.read_text(encoding='utf-8')
            head, rest = text.split(',', 1)
            comma_count = rest.count(',') + 1
            if head == interval and comma_count == n_obs:
        ...
        if i % progress_every == 0 or i == total:
            print(f'validated\t{i}/{total}\tvalid={len(valid)}\tinvalid={len(invalid)}', flush=True)
    VALIDATED_TXT.write_text('\n'.join(valid) + ('\n' if valid else ''), encoding='utf-8')
...
def main() -> None:
    ...
    parser.add_argument('--validate-progress-every', type=int, default=5000)
    args = parser.parse_args()

    requested_intervals = load_intervals(args.limit)
    ...
    state = {
        'n_obs': n_obs,
        'requested_intervals': len(requested_intervals),
    ...
    intervals, invalid = validate_intervals(requested_intervals, n_obs, args.validate_progress_every)
    state['valid_intervals'] = len(intervals)
    state['invalid_intervals'] = len(invalid)
    state['stage'] = 'building'
    state['last_updated_at'] = time.strftime('%Y-%m-%d %H:%M:%S')
    save_state(state)

    init_output(n_obs, len(intervals))
    state['n_vars'] = len(intervals)
    state['next_col'] = 0
    save_state(state)
    ...
    save_state(state)
    print(f'saved\t{OUT_H5AD}', flush=True)
