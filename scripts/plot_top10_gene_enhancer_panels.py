...
TOP_GENE_PATH = ROOT / "plot" / "gene_batch_rectified_h_gene_divergence_absolute_all15038.tsv"
OUT_DIR = ROOT / "plot" / "top10_gene_divergence_panels"
...
RNA_CMAP = LinearSegmentedColormap.from_list(
    "rna_top10", ["#F7F7F7", "#DCEAF7", "#6BAED6", "#2171B5", "#08306B"], N=256
)
...
MASK_CMAP = LinearSegmentedColormap.from_list("mask_top10", ["#EDEDED", "#0F4D92"], N=2)
...
def robust_vmax(values, percentile=99.5):
    ...
    positive_vals = values[np.isfinite(values) & (values > 0)]
    return float(np.percentile(positive_vals, percentile)) if positive_vals.size else 1.0
...
def main():
    ...
    top = top[(top["rank_eligible"] == True) & top["absolute_rank"].notna()].copy()
    top = top.sort_values("absolute_rank").head(10)
    ...
    for rank, gene in enumerate(genes, start=1):
        ...
        ncols = 8
        n_pair_cols = ncols // 2
        nrows = 1 + int(np.ceil(n_pairs / n_pair_cols))
        ...
        for idx, item in enumerate(rows):
            rr = 1 + idx // n_pair_cols
            cc = (idx % n_pair_cols) * 2
        ...
        for j in range(total):
            ...
            if rj == 0 and cj >= 2:
                continue
            pair_index = (rj - 1) * n_pair_cols + (cj // 2)
            is_used = (rj >= 1) and (pair_index < n_pairs)
            if not is_used:
                axes[rj, cj].axis("off")
        ...
        fig.suptitle(
            f"Rank {rank} {gene}: rectified_h gene / latent mask / raw and latent enhancer panels",
            x=0.03, ha="left", fontsize=10.5, fontweight="bold",
        )
        fig.text(
            0.03, 0.03,
            f"Enhancers were taken from region_to_gene_adj.tsv, filtered by initial rho >= {INITIAL_RHO_THRESHOLD:.1f} and rectified raw rho >= {RECTIFIED_RAW_RHO_THRESHOLD:.1f}, "
            f"then ordered by difference_structure_score. Raw panels show original link rho (initρ) and raw rectified spatial correlation (rectρ). "
            f"Latent panels show latent-state spatial correlation (latentρ) and enhancer-level difference score (diffS).",
